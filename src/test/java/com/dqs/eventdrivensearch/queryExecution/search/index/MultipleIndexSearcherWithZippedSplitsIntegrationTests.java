...
@Testcontainers
@ActiveProfiles("test")
public class MultipleIndexSearcherWithZippedSplitsIntegrationTests {
    @Autowired
    ...
    private final String s3SplitPath = "https://test-bucket/dqs-indexes/jpmc/splits_9_12_zip";
    ...
    public void setup() throws URISyntaxException {
        Path sampleSplit = Paths.get(getClass().getClassLoader().getResource("index.zip").toURI());
        URI s3Uri = URI.create(s3SplitPath);
        String bucketName = s3Uri.getHost().split("\\.")[0];
        String key = s3Uri.getPath().substring(1);

        s3Client.createBucket(CreateBucketRequest.builder().bucket(bucketName).build());
        s3Client.putObject(PutObjectRequest.builder().bucket(bucketName).key(key).build(),
                RequestBody.fromFile(sampleSplit));
    }
    ...
    public void shouldSearchOnIndexAndWriteResultToS3() throws ParseException {
        ...
        searcher.searchV3("Historical", queryId, indexPaths, subQueryId);
